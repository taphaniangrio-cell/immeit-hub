...
async function getArticles(filters = {}) {
  ...
  const result = await query(
    `SELECT id, titre_interne, statut, date_creation, ia_provider, ia_model, generation_type, custom_subject, source_news_titre, image_url FROM articles${whereClause} ORDER BY date_creation DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
    [...params, limit, offset]
  );
