...
function findChanges(oldItems, newItems, headers) {
  ...
  for (const [id, newItem] of newMap) {
    ...
    for (const field of WATCHED_FIELDS) {
      ...
      const digOld = digest(rawOld);
      const digNew = digest(rawNew);
      if (digOld === digNew) continue;
      log('info', 'diff_field_changed', { id, field, oldLen: rawOld.length, newLen: rawNew.length, oldDigest: digOld.slice(0, 60) + '...', newDigest: digNew.slice(0, 60) + '...', oldStart: rawOld.slice(0, 100), newStart: rawNew.slice(0, 100) });
      fieldChanges.push({
        field,
