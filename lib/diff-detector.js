...
function findChanges(oldItems, newItems, headers) {
  ...
  for (const [id, newItem] of newMap) {
    ...
    for (const field of WATCHED_FIELDS) {
      ...
      const cleanOld = rawOld.replace(/\uFFFD/g, '');
      const cleanNew = rawNew.replace(/\uFFFD/g, '');
      if (cleanOld === cleanNew) continue;
      const asciiOld = rawOld.replace(/[^\x20-\x7E]/g, '');
      const asciiNew = rawNew.replace(/[^\x20-\x7E]/g, '');
      if (asciiOld === asciiNew) continue;
      log('debug', 'diff_field_changed', { id, field, rawOldLen: rawOld.length, rawNewLen: rawNew.length, oldStart: rawOld.slice(0, 80), newStart: rawNew.slice(0, 80) });
      fieldChanges.push({
        field,
  ...
  try {
    ...
    if (result && result.rows && result.rows.length > 0) {
      ...
      if (data && data.items && data.items.length > 0) {
        ...
        log('debug', 'diff_prev_loaded_db', { items: data.items.length, syncedAt: data.syncedAt });
        return data;
      }
    }
  } catch (e) {
